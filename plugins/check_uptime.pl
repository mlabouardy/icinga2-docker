...
#                       Addev version history you're reading right now.
...
sub print_usage {
    print "Usage: $0 [-v] [-T local|unix-host|unix-sys|win] [-H <host> (-C <snmp_community>) [-2] | (-l login -x passwd [-X pass -L <authp>,<privp>) [-p <port>]] [-w <warn minutes> -s <crit minutes>] [-f] [-P <previous perf data from nagios \$SERVICEPERFDATA\$>] [-t <timeout>] | [-V] [--label <string>]\n";
}
...
sub help {
   ...
   print "GPL licence, (c) 2008-2009 William Leibzon\n\n";
   print_usage();
   print <<EOT;
...
Standard Options:
 -T, --type=auto|local|unix-host|unis-sys|windows
   Type of system:
     ...
     windows   : SNMP check from sysUptime ($oid_sysUptime) OID
...
if ($check_type==1) {  # local
  ...
  if ($uptime_output =~ /(\d+)\s+days?,\s+(\d+)\:(\d+)/) {
     ($days, $hrs, $mins) = ($1, $2, $3);
     $uptime_info = "up $days days $hrs hours $mins minutes";
  }
  elsif ($uptime_output =~ /up\s+(\d+)\shours?\s+(\d+)/) {
     ($days, $hrs, $mins) = (0, $1, $2);
     $uptime_info = "up $hrs hours $mins minutes";
  }
  ...
  if ($check_type==0) {
      $oid=$oid_hostUptime if $check_type==0; #still auto, will try host first
  }
  else {
...
verb("Uptime Text: $uptime_info") if $uptime_info;
